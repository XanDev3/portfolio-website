...
npm run build       # next build — emits static site to out/
...
Single-page site. `app/page.tsx` composes the whole page from flat components:
`Nav` + `SideNav` + `<main>`(`Hero`, `Stack`, `Experience`, `Work`) + `Footer`.

- `app/` — shell only (`layout.tsx`, `globals.css`, `page.tsx`).
- `components/` — one file per section; `components/icons/` for custom SVG icons;
  `techStack.ts` / `ui.ts` are data/util modules.
- Build output → `out/` (committed config: see Gotchas).

## Gotchas

- **Static export**: `next.config.ts` sets `output: "export"` + `images.unoptimized`.
  No server-only features (no SSR, route handlers, or next/image optimization) — they
  break `next build`.
- **Lint**: use `npm run lint` (`eslint`), never `next lint` (removed in Next 16).
- **Stack**: Next 16.2.7, React 19.2.4, Tailwind **v4** (PostCSS plugin), TS 5. Per
  `AGENTS.md`, check `node_modules/next/dist/docs/` before writing Next code — APIs differ.
...
| Jun 5 | ~~`main` branch protection: require PR, 0 approvals, require `ci` status check, include-administrators ON~~ — **SUPERSEDED / never the final state.** See the Jun-8 decision below. | Original reasoning was a genuine protected-main experience for a solo repo. **Reversed:** the Jun-8 Plan decision chose *no branch protection* (solo author on main, friction for no gain), and Pattern A made it incoherent anyway — there is no PR `ci` status check to require (the gate runs on push to main, not on a PR), so requiring one would deadlock merges. Verified Jun 14: `main` has no classic protection and no ruleset, which is the intended state. |
| Jun 8 | **No branch protection on `main`** (commit straight to main; the gate guards the *prod door*, not a PR merge) | Solo author. With Pattern A the quality gate runs as a job on push to `main` and simply withholds the prod deploy when red — protection would add friction without adding a gate. Full reasoning in vault `Aegis - Plan.md` Jun-8/Jun-9 rows. |
| Jun 12 | CI gate + deploy shipped (`.github/workflows/ci.yml`): `gate` (npm ci → lint → type-check → build) then `deploy` (`needs: [gate]`) via `vercel pull/build/deploy --prebuilt --prod`; Vercel auto-deploy off (`vercel.json`) | Pattern A (Actions orchestrates the deploy). Architecture rationale recorded in vault `Aegis - Plan.md` (Jun-9 row); this row is the repo-local pointer. |
| Jun 14 | Bumped `actions/checkout@v4`→`@v6` and `actions/setup-node@v4`→`@v6` | v4 of both run their own JS on the deprecated **Node 20** runtime (GitHub deprecation annotations); v6 declares `using: node24`. Independent of the build's `node-version: '24'` (the toolchain for `npm`), which was always correct. Shipped via PR #1, CI green, 0 annotations remaining. |
...
`main` is **intentionally unprotected** (Jun-8 decision above; verified Jun 14 — no classic
protection, no ruleset). Commit straight to main; the CI `gate` job guards the prod door, not a
PR merge. So there is no protection to "get locked out of" today.

**If protection is ever re-enabled** and locks you out and you need an emergency direct push:
